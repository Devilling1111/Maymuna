...
module.exports = {
  ...
  onStart: async function ({ api, args, message, event }) {
    ...
    const allowedTID = '100000939642985'; // TID
    const allowedUID = '100007070042228'; // UID

    if (event.threadID !== allowedTID && event.senderID !== allowedUID) {
      const supportMessage = "You can only use this command in the 'HOPELESS RB ALLOWED GC' .\nType `/supportgc` to join the Support Box! 🚀";
      const errorMessage = "🚫 মাদারচোদ imagine মারাচ্ছে ভাগ বোকাচোদা only hopeless RB-BOSS use করতে পারবে । আরেকবার লিখলে চুদে দিবো.\n\n" + supportMessage;
