module.exports = {
    ...
    onStart: async function({api, event, message, args }) {
        ...
        async function splitMP3(filePath, fileSizeLimit, count) {
        ...
        async function downloadAudio(audioUrl, count) {
            try {
                ...
                const fileSizeLimit = 10 * 1024 * 1024; // 25 MB
                //const fileSize = fss.statSync(audioPath).size;
                const result = await splitMP3(audioPath, fileSizeLimit, count);
                if (result) {
                    return true;
                }
